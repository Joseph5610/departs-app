...
export function processStops(allStops: AppStopFeature[]): AppStopFeature[] {
    ...
    for (const f of allStops) {
        ...
        if (type === 1) {
            ...
            let features = stationChildrenFeatures.get(p.parent_station);
            if (!features) {
                features = [];
                stationChildrenFeatures.set(p.parent_station, features);
            }
            features.push(f);
    ...
    // Phase 2: Iterate public stops, enrich them with colors, and apply logical grouping.
    for (const f of publicStops) {
        ...
        let hasA = 0, hasB = 0, hasC = 0, isTrain = 0;
        ...
        for (const l of lines) {
            ...
            if (t === 'metro') {
                metroSet.add(l.name);
                if (l.name === 'A') hasA = 1;
                else if (l.name === 'B') hasB = 1;
                else if (l.name === 'C') hasC = 1;
        ...
        if (type === 1) {
            ...
            let aggHasA = 0, aggHasB = 0, aggHasC = 0, aggIsTrain = 0;
            ...
            for (const cf of childFeatures) {
                for (const l of (cf.properties.lines ?? [])) {
                    ...
                    if (t === 'metro') {
                        ...
                        if (l.name === 'A') aggHasA = 1;
                        else if (l.name === 'B') aggHasB = 1;
                        else if (l.name === 'C') aggHasC = 1;
                    } else if (t === 'train' || t === 'rail') {
                        aggIsTrain = 1;
                    }
                }
            ...
            groups.set(`metro_station_${stopId}`, {
                ...
                properties: {
                    ...
                    metro_a: aggHasA as 0 | 1,
                    metro_b: aggHasB as 0 | 1,
                    metro_c: aggHasC as 0 | 1,
    ...
    // Phase 3: Flatten groups and generate artificial centroids for map rendering
    ...
    for (const groupFeatures of nameGroups.values()) {
        ...
        for (const f of groupFeatures) { 
            ...
            for (const m of (f.properties.metro_lines || [])) {
                ...
                if (m.name === 'A') aggHasA = 1;
                else if (m.name === 'B') aggHasB = 1;
                else if (m.name === 'C') aggHasC = 1;
            }
        }
