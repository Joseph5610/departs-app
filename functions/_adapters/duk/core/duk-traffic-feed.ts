...
/**
 * Fetches, validates and caches the Portabo traffic feed, shared by the vehicles and debug paths.
 * Throws when the upstream is unreachable; an empty list (e.g. overnight) is a valid result.
 */
export async function getDukTrafficFeed(city: CityConfig): Promise<DukVehicleReport[]> {
    const baseUrl = city.adapterConfig?.baseUrl;
    if (!baseUrl) {
        throw new ApiError(`No baseUrl configured for city: ${city.slug}`, 501);
    }

    const reports = await CacheManager.getOrFetch<DukVehicleReport[] | null>(
        `duk_traffic_${city.slug}`,
