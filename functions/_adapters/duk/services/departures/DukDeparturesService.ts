...
import type { Env, AppDeparture, AppDepartureResponse, AppVehicleProperties } from "../../../../_core/types";
import { ApiError } from '../../../../_core/errors';
import { ERROR_MESSAGES } from '../../../../_core/config';
import { departuresQuerySchema, parseSearchParams } from '../../../../_core/schemas';
import { normalizeRouteType } from '../../../../_core/utils/routeTypes';
import { DeparturesService } from '../../../gtfs/services/departures/DeparturesService';
import { DeparturesMapper } from '../../../gtfs/services/departures/DeparturesMapper';
import type { GtfsDepartureTuple } from '../../../gtfs/services/departures/types';
import { getGtfsRoutes, type GtfsRoute } from '../../../gtfs/core/gtfs-data';
import { getDukStationBoard, getDukUnplacedLines, surveyDukUnplacedLines, type DukBoardDeparture } from '../../core/duk-station-board';
 ...
 */
export class DukDeparturesService extends DeparturesService {
    override async getDepartures(ctx: EventContext<Env, string, unknown>): Promise<AppDepartureResponse> {
        const { stopId: stopIds } = parseSearchParams(new URL(ctx.request.url).searchParams, departuresQuerySchema);
        ...
        const parentToChildMap = await this.getParentChildMap(staticDataUrl).catch(() => ({} as Record<string, string[]>));
        ...
        const [boards, tuples, { routes }, vehiclesByTrip] = await Promise.all([
            ...
            this.fetchDepartureTuples(nodes, new Map(nodes.map(n => [n, n])), staticDataUrl).catch((e) => {
     ...
     */
    private async getBoard(request: StopRequest, platforms: string[], ctx: EventContext<Env, string, unknown>): Promise<DukBoardDeparture[] | null> {
    ...
    private async timetableOnly(
        ...
        return { departures: DeparturesMapper.mapDepartures(deps, routes, await this.getRealtimeVehiclesCache()) };
    }

    private async getVehiclesByTrip(): Promise<Map<string, AppVehicleProperties>> {
        const collection = await this.getRealtimeVehiclesCache();
