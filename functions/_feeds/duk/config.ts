/**
 * Configuration for Ústecký kraj (DÚK): static data built from the national JDF
 * export (departs-data `build-duk.mjs`) plus the Portabo realtime traffic feed.
 */
export const DUK_CONFIG = {
    /** Track files store coordinates as integers at this scale; mirrors TRACK_COORD_SCALE in departs-data. */
    TRACK_COORD_SCALE: 1_000_000,
    HOURS_PER_DAY: 24,
    /** Hour files an isolate keeps decoded; each is a few hundred kilobytes. */
