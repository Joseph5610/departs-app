...
export const VehicleDetail = React.memo<VehicleDetailProps>(({
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex flex-row md:flex-col items-center md:text-center p-4 md:p-6 bg-muted/30 rounded-3xl border border-border relative overflow-hidden gap-3 md:gap-4">
                <div
                ...
                <div className="z-10 flex-1 min-w-0 md:w-full">
                    ...
                    <div className="flex items-center md:justify-center gap-2 flex-wrap">
                        ...
                        {(vehicleDetail?.origin_timestamp || selectedVehicle?.origin_timestamp) && liveDataAgeSeconds !== null && (
                            <div className="flex items-center gap-1.5 px-2.5 py-1 bg-muted/30 rounded-full border border-border">
            ...
            {(vehicleDetail?.vehicle_descriptor?.operator || selectedVehicle?.vehicle_descriptor?.operator) && (
                <div className="flex gap-2">
                    <div className="flex-1 min-w-0 p-3 bg-muted/30 rounded-2xl border border-border flex items-center justify-between">
                    ...
                    {(vehicleDetail?.run_number || selectedVehicle?.run_number) && (
                        <div className="flex-initial min-w-[70px] p-3 bg-muted/30 rounded-2xl border border-border flex items-center gap-2">
            ...
            {vehicleDetail?.stop_times?.features && vehicleDetail.stop_times.features.length > 0 && (
                <div className="space-y-3">
                    <div className="flex items-center justify-between px-1 gap-2">
                        <span className="text-muted-foreground text-[10px] uppercase font-black tracking-widest truncate">{t('map.vehicleDetails.routeSchedule')}</span>
                        <button
                            onClick={handleTogglePastStops}
                            className="flex items-center gap-1.5 px-2 py-1 bg-muted/30 hover:bg-muted/50 rounded-xl border border-border transition-colors shrink-0"
                        >
                            ...
                            {showPastStops ? <ChevronUp size={12} className="text-muted-foreground" /> : <ChevronDown size={12} className="text-muted-foreground" />}
                        </button>
                    </div>
