...
export const VehicleMonitorRow: React.FC<VehicleMonitorRowProps> = React.memo(({ item }) => {
    ...
    const handleClick = () => {
        setIsFollowing(true);
        if (item.gtfsTripId) {
            navigate(paths.trip(selectedCity, item.gtfsTripId, item.vehicleId));
        }
    };

    const delaySeconds = item.delay;
    const delayMinutes = delaySeconds !== null ? Math.round(Math.abs(delaySeconds) / 60) : 0;
    const delayStatus = delaySeconds !== null ? getDelayStatus(delaySeconds) : 'onTime';
            ...
            <div className="flex items-center gap-2 shrink-0">
                {delaySeconds === null ? (
                    <Badge variant="outline" className="text-[10px] font-medium text-muted-foreground bg-muted/20 border-transparent px-1.5 py-0.5">
                        {t('map.vehicleDetails.notAvailable')}
                    </Badge>
                ) : (
                    <Badge
                        variant="outline"
                    ...
                    >
                        {isLate
                            ? `+${t('map.departures.minutes', { count: delayMinutes || 1 })}`
                            : isEarly
                                ? `-${t('map.departures.minutes', { count: delayMinutes || 1 })}`
                                : t('stats.onTime')}
                    </Badge>
