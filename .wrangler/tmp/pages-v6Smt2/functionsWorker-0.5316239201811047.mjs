...
// ../.wrangler/tmp/bundle-YUHZgw/checked-fetch.js
...
var onRequest = /* @__PURE__ */ __name(async (context) => {
  ...
  if (!stopId) {
    return new Response("Missing stopId", { status: 400 });
  }
  const golemioUrl = new URL("https://api.golemio.cz/v2/public/departureboards");
  const stopIds = stopId.split(",");
  const stopIdsParam = JSON.stringify({ "0": stopIds });
  golemioUrl.searchParams.append("stopIds[]", stopIdsParam);
  golemioUrl.searchParams.set("limit", "15");
  golemioUrl.searchParams.set("minutesAfter", "60");
  golemioUrl.searchParams.set("minutesBefore", "0");
  try {
    ...
    const flatDepartures = (data[0] || []).map((item) => ({
      // LIVE time (predicted or scheduled if no live data)
      timestamp: item.departure.timestamp_predicted || item.departure.timestamp_scheduled,
      // ORIGINAL time from schedule
    ...
    return new Response(JSON.stringify({ departures: flatDepartures }), {
      headers: {
        "Content-Type": "application/json",
...
var onRequest2 = /* @__PURE__ */ __name(async (context) => {
  const { env } = context;
  const fetchPage = /* @__PURE__ */ __name(async (offset) => {
  ...
  try {
    ...
    const allFeatures = [...p1.features || [], ...p2?.features || []];
    const filteredFeatures = allFeatures.filter((f) => {
      const p = f.properties;
      if (p.location_type === 1) return true;
      if (p.location_type === 2) return true;
      if (p.location_type === 0 && p.parent_station) return false;
      if (p.zone_id === null) return false;
      return true;
    });
    const groups = {};
    for (const f of filteredFeatures) {
      ...
      const platform = p.platform_code || "";
      const type = p.location_type;
      const key = type === 2 ? `entrance_${p.stop_id}` : `${name}_${platform}_${type}`;
      if (!groups[key]) {
        groups[key] = {
          ...f,
          properties: { ...f.properties, all_ids: [f.properties.stop_id] }
        };
      } else {
        groups[key].properties.all_ids.push(f.properties.stop_id);
      }
    }
    const mappedFeatures = Object.values(groups).map((f) => ({
      type: "Feature",
      geometry: f.geometry,
      properties: {
        ...f.properties,
        stop_id: f.properties.all_ids.join(",")
        // Join IDs with comma for the API
      }
    }));
...
// ../.wrangler/tmp/pages-v6Smt2/functionsRoutes-0.5320229854750197.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-YUHZgw/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-YUHZgw/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.5316239201811047.mjs.map
