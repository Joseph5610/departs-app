...
- **NEVER** introduce a loose `const` for a tunable value. Config belongs in a config object: `_core/config.ts` for cross-network values, `_feeds/<network>/config.ts` for network-specific ones.
